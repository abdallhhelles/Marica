...
- **Profiles:** `/profile` for XP + stash + scan summary.
...
- **Guides:** `/commands`, `/features`, and `/heroes` for onboarding.

### 4) Automation & passive features
- **Event reminders:** A channel announcement fires at 60 minutes (with @everyone). All other reminders are DMs to users who reacted to opt in. All messages follow Marcia's personality: clear, calm, firm, and encouraging.
...
- **Remove an event:** `/event` → **Remove Event**
- **List upcoming events:** open `/event` and tap **Upcoming Events**
- **Join Event tracking:** at the 60-minute notification, members can react with 🤝 to join; reminders are DM-only after that.
...
4. **Participation tracking:** members who react with 🤝 are marked as participants and receive DM-only reminders.
...
- **OCR dependencies:** Tesseract+pytesseract cover basic scans. For higher accuracy, install the OCR extras bundled in `requirements.txt` (easyocr, opencv, numpy) unless memory is constrained; on tiny hosts you can skip them with `requirements-lite.txt` (scanning stays disabled). See [Low-memory installation](LOW_MEMORY_INSTALL.md) for preloading wheels when RAM is limited.
...
2. **Configure channel:** Run `/setup` → **Profile scan intake** in a test server and confirm the setup embed.
...
4. **Profile view:** Run `/profile` and confirm it matches the prior reply, including the `Last scanned` timestamp and verification status.
