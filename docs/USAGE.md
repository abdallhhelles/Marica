...
- **Compare stats:** `/leaderboard` opens a menu for XP plus CP/kills/likes/VIP/level from scanned profiles.
...
5. **Leaderboard:** Run `/leaderboard` and pick **Combat Power** (and other stats if populated). Verify ordering matches expectations and users without values are skipped.
